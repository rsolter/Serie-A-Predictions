## This script gathers Serie A match results for 19 teams from 2000 to 2019.
...
teams <- c("juventus","atalanta","sampdoria","empoli-fc",
           "udinese-calcio","ac-milan","acf-fiorentina",
           "chievo-verona","ssc-napoli","bologna-fc","inter",
           "parma-calcio-1913","frosinone-calcio","cagliari-calcio",
           "torino-fc","sassuolo-calcio","spal-2013-ferrara","as-roma",
           "genoa-cfc")
years <- 2000:2019
...
for (j in teams[13:19]){
  for (i in years){
    ...
    x<-grepl("Serie A", temp_hist[,"X1"], ignore.case=TRUE)
    begin<-match(T,x)+1
    ...
    y<-grepl("Coppa", temp_hist[,"X1"], ignore.case=TRUE)
    end<-match(T,y)-1
