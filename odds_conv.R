library(readxl)
library(tidyverse)
odds<-readxl::read_excel("C:/Users/rsolt008/Documents/odds.xlsx",sheet = 2)

odds$`1` <- as.integer(odds$`1`)
odds$`2` <- as.integer(odds$`2`)
odds$X <- as.integer(odds$X)

odds_to_perc <- function(x){
  if(x>0){
    y <- 100/(x+100)
  } else {
    x <- abs(x)
...
odds_to_perc_v <-Vectorize(odds_to_perc)

odds$perc_1 <- odds_to_perc_v(odds$`1`)
odds$perc_X <- odds_to_perc_v(odds$X)
odds$perc_2 <- odds_to_perc_v(odds$`2`)

odds <- odds %>% mutate(total=perc_1+perc_2+perc_X)
