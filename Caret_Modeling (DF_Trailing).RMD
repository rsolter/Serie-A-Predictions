...
Next we nest the larger dataset into a list of dataframes for each individual team
```{r Nesting Data}
...
# printing out the number of records for each DF. Notice that teams that have been in SerieA for the past 3+ years will have 100 records, while Empoli and Spal only have 62 records
for(i in 1:length(nested_DF)){ print(dim(nested_DF[[i]]))}
...
# overall avg weekly accuracy is 50% when the TRCONTROL window is not fixed (growwing)
# It's slightly higher at 53 when is fixed
