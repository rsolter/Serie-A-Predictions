...
teams <- c("afc-bournemouth","arsenal-fc","brighton-hove-albion","burnley-fc","cardiff-city"
           ,"chelsea-fc","crystal-palace","everton-fc","fulham-fc","huddersfield-town"
           ,"leicester-city","liverpool-fc","manchester-city","manchester-united","newcastle-united"
           ,"southampton-fc","tottenham-hotspur","watford-fc","west-ham-united","wolverhampton-wanderers")
years <- 2000:2019
...
for (j in teams){
  for (i in years){
    ...
    x<-grepl("Premier", temp_hist[,"X1"], ignore.case=TRUE)
    begin<-match(T,x)+1
    ...
    y<-grepl("FA", temp_hist[,"X1"], ignore.case=TRUE)
    end<-match(T,y)-1
...
save(final_df_full,file="Personal_Git//Premhist.rdata")
